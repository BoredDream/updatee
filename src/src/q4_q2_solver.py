...
def price_forecast_145(data, day: int, window: int = 20):
    """完整历史行最晚取至day-2，午夜参考取上一已完成时段，返回145段预测。"""
    p4, p1 = data["price4_template"], data["price_template"]
    end = day - 1  # exclusive；day-1行末列在当天0:00尚未实现
    if end <= 0:
        # 1月1日与1月2日均无完整历史模板行，使用附件1价格先验。
        template = p1.copy()
    else:
        hist = p4[max(0, end-window):end]
        shape = hist.mean(0) / max(float(hist.mean()), 1e-9)
        level = 0.5*float(p4[end-1].mean()) + 0.5*float(hist.mean())
        template = np.maximum(shape*level, 1e-3)
    midnight = float(p4[day-1, 142]) if day > 0 else float(p1[-1])
