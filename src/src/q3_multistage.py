...
PV_BLEND_PRIOR = 0.5                    # 无历史时等权；不使用评价期离线调参
PV_BLEND_WINDOW = 30                    # 仅用决策时已完整实现的滚动历史日
_PV_WEIGHT_CACHE = {}
...
def pv_from_forecast(d, m):
    ...
    if h0 == 0:
        ...
        # 模板下标 TSTAGE[m] 正是以发布时刻为起点、尚未实现的区间；
        # 因此锚点必须取它前一段，而不是用 h0*6-1 误取当前区间。
        anchor = GKW[d, TSTAGE[m] - 1]
...
def latest_complete_template_day(d, m):
    """决策时最后一个完整模板行下标。

    模板末列属于次日00:00--00:10：在0:00尚未实现，所以阶段0只能用到d-2；
    6/12/18点时该午夜段已经完成，可以用到d-1。
    """
    return d - 2 if m == 0 else d - 1


def pv_decay(d, m=0, lam=0.8, n=10):
    """近 n 日指数衰减加权平均(kW)，作为与附件3独立的第二个预测源。"""
    latest = latest_complete_template_day(d, m)
    idx = [latest-j for j in range(n) if latest-j >= 0]
    if not idx:
        return _a1.G.values.copy()
    w = np.array([lam**j for j in range(len(idx))]); w /= w.sum()
    return (GKW[idx] * w[:, None]).sum(0)

def pv_blend_weight(d, m):
    """用决策时已完整实现的历史日滚动估计组合权重，避免评价期调参泄露。"""
    key = (d, m)
    if key in _PV_WEIGHT_CACHE:
        return _PV_WEIGHT_CACHE[key]
    latest = latest_complete_template_day(d, m)
    start = max(0, latest - PV_BLEND_WINDOW + 1)
    issue = TSTAGE[m]
    numer = denom = 0.0
    for dd in range(start, latest + 1):
        fc = pv_from_forecast(dd, m)
        if fc is None:
            continue
        dc = pv_decay(dd, m)
        delta = fc[issue:] - dc[issue:]
        target = GKW[dd, issue:] - dc[issue:]
        numer += float(delta @ target)
        denom += float(delta @ delta)
    weight = PV_BLEND_PRIOR if denom <= 1e-12 else float(np.clip(numer / denom, 0.0, 1.0))
    _PV_WEIGHT_CACHE[key] = weight
    return weight


def clear_forecast_caches():
    """供因果扰动测试在修改输入后清除派生量缓存。"""
    _PV_WEIGHT_CACHE.clear()


def pv_hat(d, m):
    """组合预测：权重仅由发布时刻之前已完整实现的历史滚动估计。"""
    fc = pv_from_forecast(d, m)
    dc = pv_decay(d, m)
    if fc is None:
        return dc * DT
    w = pv_blend_weight(d, m)
...
def load_hat(d, m):
    ...
    r = []
    # 阶段0不能读取d-1模板末列（它是当天00:00--00:10）；日内阶段可以。
    first_lag = 2 if m == 0 else 1
    for j in range(first_lag, first_lag + 7):
        dd = d - j
