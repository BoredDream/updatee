...
def main() -> int:
    p2, d2, by2 = load("2")
    p3, d3, by3 = load("3")
    expected_price_policy = M.price_objective_policy()
    summary3 = json.loads((OUT / f"summary_q4-3_K{K}.json").read_text(encoding="utf-8"))
    for artifact, meta in (("payload", p3["meta"]), ("summary", summary3["meta"])):
        actual_price_policy = {k: meta.get(k) for k in expected_price_policy}
        if actual_price_policy != expected_price_policy:
            raise ValueError(
                f"Q4-3 {artifact} 价格目标元数据不一致："
                f"{actual_price_policy!r} != {expected_price_policy!r}"
            )
    ...
    A("4-2保留Q2的负荷/光伏预测器、145段时域、SOC与因果执行规则，只加入基于历史的波动价格预测；"
      ...
      f"（情景数 `K = {K}`）。")
    A("")
    A("4-3有意采用**中心预测价 + 场景追索价**的混合目标：不带场景下标的共享合同决策"
      "使用当前阶段中心预测价，未来合同、超计划调整与紧急购电等价格相关追索项使用对应场景价格。价格乘性残差"
      "不重新中心化，故中心预测价不必等于场景均价；该口径不等同于把所有价格项统一置于"
      "同一经验分布下的严格 SAA 期望费用。")
    A("")
    ...
    if Q3_SUMMARY.exists():
        ...
        A("附件1 的分时电价恰是附件4 电价过程的**逐时段均值**（逐时段最大差 5.15e-05，"
          "属表格取整）。这是两份输入数据的样本均值事实；由于两问的价格信息、预测方式及"
          "Q4-3混合价格目标不同，下面只能作为两套完整策略的描述性对照，不能当作同一优化"
          "目标下的严格控制变量实验：")
        A("")
        ...
        A("")
        A("即：**在两套模型及各自信息结构下，波动电价方案的交付期总费用比确定性电价方案高 "
          f"{(t3['total_cost_yuan'] / q3['total_cost_yuan'] - 1) * 100:.2f}%**。")
        A("")
        ...
        A("> 信息缺失成本。第10节在相应证据产物可用时给出「完美预见电价」回测（把 `p̂` 换成")
        A("> 当日实际电价、其余不动），用于辅助拆分这两部分。")
        A("")
    ...
    if worse:
        A(f"需要说明：这四个日期里有 {len(worse)} 天（{'、'.join(worse)}）4-3 的当日总费用"
          ...
          "用已知信息纠正 0:00 计划，它在全周期的混合场景目标上寻求降本，不保证每一天都更优；且调整本身要付"
