...
def solver_section(s: dict | None, by: dict) -> list[str]:
    ...
    A("本节检验求解器配置对现行模型最优值与实际决策的影响，"
      "脚本为 `scripts/q3_solver_uniqueness_check.py`。")
    A("")
...
def main() -> int:
    ...
    A("每天 0:00 制定当天计划，**6:00 / 12:00 / 18:00 三次**仅调整尚未交付的区间；"
      "本阶段锁定的合约量由全部情景共享，后续可调整合约及储能、紧急购电、"
      "弃电和 SOC 均作为情景补救变量；实际充放电按已实现的供需缺口执行。")
    A("")
    ...
    A("## 3. 结果来源与比较范围")
    A("")
    A("第三问统一使用 `src/q3_multistage.py`，结果读取自 `outputs/q3_multistage/`。"
      "计划与调整量按模板行记录，物理执行及总费用按自然日统计。"
      "第 4 节只比较同一模型、相同物理参数、情景数和评价时段下的预报发布组合。")
    A("")
    A("## 4. 预报发布时刻组合对照")
    A("")
    X(comparison(comp))
